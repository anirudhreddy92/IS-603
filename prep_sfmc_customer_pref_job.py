...
# CUSTOMER_CONTACT & CUSTOMER_MOBILE_CONSENT:-
customer_contact_df = pref_df_0.filter(col("MOBILE_NUMBER").isNotNull())
customer_contact_df = customer_contact_df.select(col("GUID_CIF").alias("CUSTOMER_ID"), lit("SMS").alias("COMMUNICATION_TYPE"), col("MOBILE_NUMBER").alias("COMMUNICATION_VAL"),
                                                 lit("primary").alias("PRIORITY"))
customer_contact_df.printSchema()
print('count: {}'.format(customer_contact_df.count()))
...
print('count: {}'.format(customer_mobile_consent_df.count()))
...
pref_df_3 = pref_df_2.withColumn("ALERT_WITH_CHANNEL", split_col_with_val.getItem(0)).withColumn("FLAG", split_col_with_val.getItem(1)).drop("col_with_val")
pref_df_31 = pref_df_3.withColumn("ALERT_WITH_CHANNEL_ARRAY", split("ALERT_WITH_CHANNEL", "_")).drop("ALERT_WITH_CHANNEL")
...
pref_df.printSchema()
print('Flattened df count: {}, expected: {}'.format(pref_df.count(), 54*pref_df_0.count()))
...
print('Flattened df count: {}'.format(cap_df.count()))
...
print('Record Count from Glue staging Tables!!')
